...
`lb setup codex` also installs the bundled `brite-architect` Codex skill into `$CODEX_HOME/skills/brite-architect`, falling back to `$HOME/.codex/skills/brite-architect` when `CODEX_HOME` is unset. The installer only writes the skill when that directory is missing; if it already exists, setup leaves custom skill files untouched.
