...
`.codex/config.toml` enables Codex hooks with `[features].hooks = true`; the old `[features].codex_hooks` key is deprecated and setup commands remove it when they refresh config. `.codex/hooks.json` runs `lb prime` and `trk prime` on startup, resume, and clear so agents receive active tracker state plus the persistent wiki protocol before exploring. `.codex/rules/default.rules` allows both `lb` and `trk` commands.
